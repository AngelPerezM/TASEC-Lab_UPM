...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_send_telecommand
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_send_telecommand
    tcId = i_gui_RI_send_telecommand
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global send_telecommand_via_shared_lib
    shared_lib = True
    send_telecommand_via_shared_lib = dll.gui_PI_send_telecommand
...
def send_send_telecommand_VN(tc_gser):
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_send_telecommand(tc)
            except IOError:
                raise
    elif udp:
