...
package body Thermostat_Datamodel with SPARK_Mode is
...
end asn1SccThermostat_States_Equal;

function asn1SccThermostat_States_Init return asn1SccThermostat_States
is
    val: asn1SccThermostat_States;
begin
    val := asn1Sccwait;
	pragma Warnings (Off, "object ""val"" is always");
    return val;
	pragma Warnings (On, "object ""val"" is always");
end asn1SccThermostat_States_Init;
...
begin
    ...
    ret.ErrorCode := (if ret.Success then 0 else ERR_THERMOSTAT_STATES);
    return ret;
end asn1SccThermostat_States_IsConstraintValid;
...
begin
    ...
    if ret then
        ...
        if ret then
            ret := (adaasn1rtl.Asn1Real_Equal(val1.tempcelsius, val2.tempcelsius));

            if ret then
                ret := (val1.heaterison = val2.heaterison);
...
end asn1SccThermostat_Context_Equal;

function asn1SccThermostat_Context_Init return asn1SccThermostat_Context
is
    val: asn1SccThermostat_Context;
begin

    --set state 
    val.state := asn1SccThermostat_States_Init;
    --set init_done 
    val.init_done := FALSE;
    --set tempcelsius 
    val.tempcelsius := TASEC_LAB_B2SPACE_DATAVIEW.asn1SccT_Float_Init;
    --set heaterison 
    val.heaterison := TASTE_BasicTypes.asn1SccT_Boolean_Init;
	pragma Warnings (Off, "object ""val"" is always");
    return val;
	pragma Warnings (On, "object ""val"" is always");
end asn1SccThermostat_Context_Init;
...
is
    ...
    pragma Warnings (On, "initialization of ret has no effect");        
begin
    ret := asn1SccThermostat_States_IsConstraintValid(val.state);
    if ret.Success then
        ret := TASEC_LAB_B2SPACE_DATAVIEW.asn1SccT_Float_IsConstraintValid(val.tempcelsius);
        if ret.Success then
            ret := TASTE_BasicTypes.asn1SccT_Boolean_IsConstraintValid(val.heaterison);
        end if;
    end if;
    return ret;
end asn1SccThermostat_Context_IsConstraintValid;
