...
#ifdef __NEED_TC_Heater_UPER
int Encode_UPER_TC_Heater(void *pBuffer, size_t iMaxBufferSize, const asn1SccTC_Heater *pSrc)
{
    (void)iMaxBufferSize;
    int errorCode;
    STATIC BitStream strm;
    ...
    BitStream_Init(&strm, pBuffer, iMaxBufferSize);
    if (asn1SccTC_Heater_Encode(pSrc, &strm, &errorCode, TRUE) == FALSE) {
#ifdef __unix__
	fprintf(stderr, "Could not encode TC-Heater (at %s, %d), errorCode was %d\n", __FILE__, __LINE__, errorCode);
#endif
        return -1;
    } else {
        return BitStream_GetLength(&strm);
...
#ifdef __NEED_TC_Heater_ACN
int Encode_ACN_TC_Heater(void *pBuffer, size_t iMaxBufferSize, asn1SccTC_Heater *pSrc)
{
    (void)iMaxBufferSize;
    int errorCode;
    STATIC BitStream strm;
    ...
    BitStream_Init(&strm, pBuffer, iMaxBufferSize);
    if (asn1SccTC_Heater_ACN_Encode(pSrc, &strm, &errorCode, TRUE) == FALSE) {
#ifdef __unix__
	fprintf(stderr, "Could not encode TC-Heater (at %s, %d), errorCode was %d\n", __FILE__, __LINE__, errorCode);
#endif
        return -1;
...
int Encode_NATIVE_TC_Heater(void *pBuffer, size_t iMaxBufferSize, const asn1SccTC_Heater *pSrc)
{
    (void)iMaxBufferSize;
    memcpy(pBuffer, pSrc, sizeof(asn1SccTC_Heater) );
    return sizeof(asn1SccTC_Heater);
}
#endif

#ifdef __NEED_TC_Heater_UPER
int Decode_UPER_TC_Heater(asn1SccTC_Heater *pDst, void *pBuffer, size_t iBufferSize)
{
    (void)iBufferSize;
    int errorCode;
    ...
    if (asn1SccTC_Heater_Decode(pDst, &strm, &errorCode)) {
        /* Decoding succeeded */
        return 0;
    } else {
#ifdef __unix__
	fprintf(stderr, "Could not decode TC-Heater (at %s, %d), error code was %d\n", __FILE__, __LINE__, errorCode);
#endif
        return -1;
    }
}
...
#ifdef __NEED_TC_Heater_ACN
int Decode_ACN_TC_Heater(asn1SccTC_Heater *pDst, void *pBuffer, size_t iBufferSize)
{
    (void)iBufferSize;
    int errorCode;
    ...
    if (asn1SccTC_Heater_ACN_Decode(pDst, &strm, &errorCode)) {
        /* Decoding succeeded */
        return 0;
    } else {
#ifdef __unix__
