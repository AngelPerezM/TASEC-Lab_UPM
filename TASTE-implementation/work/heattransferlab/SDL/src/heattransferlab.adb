...
package body Heattransferlab is
   ...
   procedure configureParameters(Configuration: in out asn1SccHTL_Config) is
      tmp5999 : asn1SccT_Float;
      tmp6049 : asn1SccT_Double;
      tmp6007 : asn1SccT_Float;
      tmp6041 : asn1SccT_Double;
      tmp6033 : asn1SccT_Double;
      tmp5991 : asn1SccT_Float;
      tmp6015 : asn1SccT_Double;
      tmp6024 : asn1SccT_Double;
      begin
         ...
         if (Configuration.exist.press_5km = 1) then
            tmp5991 := Configuration.press_5km;
         else
            tmp5991 := ctxt.press_5km;
         end if;
         ctxt.press_5km := tmp5991;
         ...
         if (Configuration.exist.press_10km = 1) then
            tmp5999 := Configuration.press_10km;
         else
            tmp5999 := ctxt.press_10km;
         end if;
         ctxt.press_10km := tmp5999;
         ...
         if (Configuration.exist.press_18km = 1) then
            tmp6007 := Configuration.press_18km;
         else
            tmp6007 := ctxt.press_18km;
         end if;
         ctxt.press_18km := tmp6007;
         ...
         --   if exist(Configuration.f1_duration_secs) then
            ...
            if (Configuration.exist.f1_duration_secs = 1) then
               tmp6015 := Asn1Real(Configuration.f1_duration_secs);
            else
               tmp6015 := ctxt.f1_relative_duration_max;
            end if;
            ctxt.f1_relative_duration_max := tmp6015;
            ...
            --  if exist(Configuration.f2_duration_secs) then
               ...
               if (Configuration.exist.f2_duration_secs = 1) then
                  tmp6024 := Asn1Real(Configuration.f2_duration_secs);
               else
                  tmp6024 := ctxt.f2_relative_duration_max;
               end if;
               ctxt.f2_relative_duration_max := tmp6024;
               ...
               --  if exist(Configuration.a1_duration_emergency_secs) then
                  ...
                  if (Configuration.exist.a1_duration_emergency_secs = 1) then
                     tmp6033 := Configuration.a1_duration_emergency_secs;
                  else
                     tmp6033 := ctxt.a1_duration_emergency;
                  end if;
                  ctxt.a1_duration_emergency := tmp6033;
                  --  a1_duration_max :=
                  --  if exist(Configuration.a1_duration_max_secs) then
                     ...
                     if (Configuration.exist.a1_duration_max_secs = 1) then
                        tmp6041 := Configuration.a1_duration_max_secs;
                     else
                        tmp6041 := ctxt.a1_duration_max;
                     end if;
                     ctxt.a1_duration_max := tmp6041;
                     --  a2_duration_max :=
                     --  if exist(Configuration.a2_duration_max_secs) then
                        ...
                        if (Configuration.exist.a2_duration_max_secs = 1) then
                           tmp6049 := Configuration.a2_duration_max_secs;
                        else
                           tmp6049 := ctxt.a2_duration_max;
                        end if;
                        ctxt.a2_duration_max := tmp6049;
                  ...
                  function p_0_updateData return asn1SccT_Boolean is
                     ...
                     tmp6066 : asn1SccOBSW_DP_Filter;
                     tmp6062 : asn1SccOBSW_DP_Filter;
                     tmp6166 : asn1SccT_Float;
                     tmp6175 : asn1SccT_Float;
                     --  !! stack: _call_external_function line 1604
                     tmp6074 : asn1SccOBSW_DP_Filter;
                     tmp6187 : asn1SccT_Float;
                     tmp6154 : asn1SccT_Float;
                     tmp6201 : asn1SccT_Float;
                     tmp6070 : asn1SccOBSW_DP_Filter;
                     tmp6144 : asn1SccT_Float;
                     begin
                        --  getTime(gps_time, htl_time) (359,17)
                        RI_0_getTime(gps_time, ctxt.htl_time);
                        --  retreiveSingleData(pt1000s,pt1000s_data) (361,17)
                        tmp6062 := asn1Sccpt1000s;
                        RI_0_RetreiveSingleData(tmp6062, pt1000s_data);
                        --  retreiveSingleData(ps1, press1_data) (363,17)
                        tmp6066 := asn1Sccps1;
                        RI_0_RetreiveSingleData(tmp6066, press1_data);
                        --  retreiveSingleData(ps2, press2_data) (365,17)
                        tmp6070 := asn1Sccps2;
                        RI_0_RetreiveSingleData(tmp6070, press2_data);
                        --  retreiveSingleData(heater2, power_data) (367,17)
                        tmp6074 := asn1Sccheater2;
                        RI_0_RetreiveSingleData(tmp6074, power_data);
                        --  temp_air := 0.0 (369,17)
                        ctxt.temp_air := 0.0;
                        ...
                        if ((((power_ok and (placa_abajo_ok or placa_arriba_ok)) and aire_infinito_ok) and (press1_ok or press2_ok))) = true then
                           ...
                           if placa_arriba_ok then
                              tmp6144 := pt1000s_data.pt1000s.data.celsius.Data(6);
                           else
                              tmp6144 := 0.0;
                           end if;
                           ctxt.temp_plate := tmp6144;
                           ...
                           if placa_abajo_ok then
                              tmp6154 := (pt1000s_data.pt1000s.data.celsius.Data(7) + ctxt.temp_plate);
                           else
                              tmp6154 := ctxt.temp_plate;
                           end if;
                           ctxt.temp_plate := tmp6154;
                           ...
                           if (placa_abajo_ok and placa_arriba_ok) then
                              tmp6166 := (ctxt.temp_plate / 2.0);
                           else
                              tmp6166 := ctxt.temp_plate;
                           end if;
                           ctxt.temp_plate := tmp6166;
                           --  press := if press1_ok then float(press1_data.ps1.data.processed.pressure) /100.0 else 0.0 fi (391,0)
                           if press1_ok then
                              tmp6175 := (Asn1Real(press1_data.ps1.data.processed.pressure) / 100.0);
                           else
                              tmp6175 := 0.0;
                           end if;
                           ctxt.press := tmp6175;
                           ...
                           if press2_ok then
                              tmp6187 := ((Asn1Real(press2_data.ps2.data.processed.pressure) / 100.0) + ctxt.press);
                           else
                              tmp6187 := ctxt.press;
                           end if;
                           ctxt.press := tmp6187;
                  ...
                  procedure p_0_initA1 is
                     tmp6340 : asn1SccT_Double;
                     tmp6339 : asn1SccT_Double;
                     --  !! stack: _call_external_function line 1604
                     begin
                        ...
                        if (ctxt.a1_started) = true then
                           ...
                           tmp6339 := (-1.0);
                           tmp6340 := (-1.0);
                           RI_0_RecordHTLParams(ctxt.actual_state, tmp6339, tmp6340, ctxt.m, ctxt.n, ctxt.power_f1);
                        end if;
                  ...
                  procedure p_0_initA2 is
                     tmp6362 : asn1SccT_Double;
                     tmp6361 : asn1SccT_Double;
                     --  !! stack: _call_external_function line 1604
                     begin
                        ...
                        if (ctxt.a2_started) = true then
                           ...
                           tmp6361 := (-1.0);
                           tmp6362 := (-1.0);
                           RI_0_RecordHTLParams(ctxt.actual_state, tmp6361, tmp6362, ctxt.m, ctxt.n, ctxt.power_f1);
                        end if;
