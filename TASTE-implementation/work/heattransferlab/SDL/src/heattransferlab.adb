...
package body Heattransferlab is
   ...
   procedure configureParameters(Configuration: in out asn1SccHTL_Config) is
      tmp7921 : asn1SccT_Float;
      tmp7954 : asn1SccT_Double;
      tmp7937 : asn1SccT_Float;
      tmp7963 : asn1SccT_Double;
      tmp7929 : asn1SccT_Float;
      tmp7971 : asn1SccT_Double;
      tmp7979 : asn1SccT_Double;
      tmp7945 : asn1SccT_Double;
      begin
         ...
         if (Configuration.exist.press_5km = 1) then
            tmp7921 := Configuration.press_5km;
         else
            tmp7921 := ctxt.press_5km;
         end if;
         ctxt.press_5km := tmp7921;
         ...
         if (Configuration.exist.press_10km = 1) then
            tmp7929 := Configuration.press_10km;
         else
            tmp7929 := ctxt.press_10km;
         end if;
         ctxt.press_10km := tmp7929;
         ...
         if (Configuration.exist.press_18km = 1) then
            tmp7937 := Configuration.press_18km;
         else
            tmp7937 := ctxt.press_18km;
         end if;
         ctxt.press_18km := tmp7937;
         ...
         --   if exist(Configuration.f1_duration_secs) then
            ...
            if (Configuration.exist.f1_duration_secs = 1) then
               tmp7945 := Asn1Real(Configuration.f1_duration_secs);
            else
               tmp7945 := ctxt.f1_relative_duration_max;
            end if;
            ctxt.f1_relative_duration_max := tmp7945;
            ...
            --  if exist(Configuration.f2_duration_secs) then
               ...
               if (Configuration.exist.f2_duration_secs = 1) then
                  tmp7954 := Asn1Real(Configuration.f2_duration_secs);
               else
                  tmp7954 := ctxt.f2_relative_duration_max;
               end if;
               ctxt.f2_relative_duration_max := tmp7954;
               ...
               --  if exist(Configuration.a1_duration_emergency_secs) then
                  ...
                  if (Configuration.exist.a1_duration_emergency_secs = 1) then
                     tmp7963 := Configuration.a1_duration_emergency_secs;
                  else
                     tmp7963 := ctxt.a1_duration_emergency;
                  end if;
                  ctxt.a1_duration_emergency := tmp7963;
                  --  a1_duration_max :=
                  --  if exist(Configuration.a1_duration_max_secs) then
                     ...
                     if (Configuration.exist.a1_duration_max_secs = 1) then
                        tmp7971 := Configuration.a1_duration_max_secs;
                     else
                        tmp7971 := ctxt.a1_duration_max;
                     end if;
                     ctxt.a1_duration_max := tmp7971;
                     --  a2_duration_max :=
                     --  if exist(Configuration.a2_duration_max_secs) then
                        ...
                        if (Configuration.exist.a2_duration_max_secs = 1) then
                           tmp7979 := Configuration.a2_duration_max_secs;
                        else
                           tmp7979 := ctxt.a2_duration_max;
                        end if;
                        ctxt.a2_duration_max := tmp7979;
                  ...
                  function p_0_updateData return asn1SccT_Boolean is
                     ...
                     tmp8074 : asn1SccT_Float;
                     tmp8117 : asn1SccT_Float;
                     tmp8105 : asn1SccT_Float;
                     tmp8000 : asn1SccOBSW_DP_Filter;
                     tmp8131 : asn1SccT_Float;
                     tmp7996 : asn1SccOBSW_DP_Filter;
                     --  !! stack: _call_external_function line 1604
                     tmp8084 : asn1SccT_Float;
                     tmp8096 : asn1SccT_Float;
                     tmp8004 : asn1SccOBSW_DP_Filter;
                     tmp7992 : asn1SccOBSW_DP_Filter;
                     begin
                        --  getTime(gps_time, htl_time) (375,17)
                        RI_0_getTime(gps_time, ctxt.htl_time);
                        --  retreiveSingleData(pt1000s,pt1000s_data) (377,17)
                        tmp7992 := asn1Sccpt1000s;
                        RI_0_RetreiveSingleData(tmp7992, pt1000s_data);
                        --  retreiveSingleData(ps1, press1_data) (379,17)
                        tmp7996 := asn1Sccps1;
                        RI_0_RetreiveSingleData(tmp7996, press1_data);
                        --  retreiveSingleData(ps2, press2_data) (381,17)
                        tmp8000 := asn1Sccps2;
                        RI_0_RetreiveSingleData(tmp8000, press2_data);
                        --  retreiveSingleData(heater2, power_data) (383,17)
                        tmp8004 := asn1Sccheater2;
                        RI_0_RetreiveSingleData(tmp8004, power_data);
                        --  temp_air := 0.0 (385,17)
                        ctxt.temp_air := 0.0;
                        ...
                        if ((((power_ok and (placa_abajo_ok or placa_arriba_ok)) and aire_infinito_ok) and (press1_ok or press2_ok))) = true then
                           ...
                           if placa_arriba_ok then
                              tmp8074 := pt1000s_data.pt1000s.data.celsius.Data(6);
                           else
                              tmp8074 := 0.0;
                           end if;
                           ctxt.temp_plate := tmp8074;
                           ...
                           if placa_abajo_ok then
                              tmp8084 := (pt1000s_data.pt1000s.data.celsius.Data(7) + ctxt.temp_plate);
                           else
                              tmp8084 := ctxt.temp_plate;
                           end if;
                           ctxt.temp_plate := tmp8084;
                           ...
                           if (placa_abajo_ok and placa_arriba_ok) then
                              tmp8096 := (ctxt.temp_plate / 2.0);
                           else
                              tmp8096 := ctxt.temp_plate;
                           end if;
                           ctxt.temp_plate := tmp8096;
                           --  press := if press1_ok then float(press1_data.ps1.data.processed.pressure) /100.0 else 0.0 fi (407,0)
                           if press1_ok then
                              tmp8105 := (Asn1Real(press1_data.ps1.data.processed.pressure) / 100.0);
                           else
                              tmp8105 := 0.0;
                           end if;
                           ctxt.press := tmp8105;
                           ...
                           if press2_ok then
                              tmp8117 := ((Asn1Real(press2_data.ps2.data.processed.pressure) / 100.0) + ctxt.press);
                           else
                              tmp8117 := ctxt.press;
                           end if;
                           ctxt.press := tmp8117;
                  ...
                  procedure p_0_initA1 is
                     tmp8273 : asn1SccT_Double;
                     --  !! stack: _call_external_function line 1604
                     tmp8274 : asn1SccT_Double;
                     begin
                        ...
                        if (ctxt.a1_started) = true then
                           ...
                           tmp8273 := (-1.0);
                           tmp8274 := (-1.0);
                           RI_0_RecordHTLParams(ctxt.actual_state, tmp8273, tmp8274, ctxt.m, ctxt.n, ctxt.power_f1);
                        end if;
                  ...
                  procedure p_0_initA2 is
                     tmp8295 : asn1SccT_Double;
                     --  !! stack: _call_external_function line 1604
                     tmp8296 : asn1SccT_Double;
                     begin
                        ...
                        if (ctxt.a2_started) = true then
                           ...
                           tmp8295 := (-1.0);
                           tmp8296 := (-1.0);
                           RI_0_RecordHTLParams(ctxt.actual_state, tmp8295, tmp8296, ctxt.m, ctxt.n, ctxt.power_f1);
                        end if;
