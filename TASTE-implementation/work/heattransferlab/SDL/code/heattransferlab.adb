   ...
   procedure configureParameters(Configuration: in out asn1SccHTL_Config) is
      ...
      begin
         ...
         if (Configuration.exist.press_5km = 1) then
            tmp360 := Configuration.press_5km;
         else
            tmp360 := ctxt.press_5km;
         end if;
         ctxt.press_5km := tmp360;
         ...
         if (Configuration.exist.press_10km = 1) then
            tmp368 := Configuration.press_10km;
         else
            tmp368 := ctxt.press_10km;
         end if;
         ctxt.press_10km := tmp368;
         ...
         if (Configuration.exist.press_18km = 1) then
            tmp376 := Configuration.press_18km;
         else
            tmp376 := ctxt.press_18km;
         end if;
         ctxt.press_18km := tmp376;
         --  f1_duration :=
         --   if exist(Configuration.f1_duration_secs) then
            ...
            if (Configuration.exist.f1_duration_secs = 1) then
               tmp384 := (Configuration.f1_duration_secs * 1000);
            else
               tmp384 := ctxt.f1_duration;
            end if;
            ctxt.f1_duration := tmp384;
            --  f2_duration :=
            --  if exist(Configuration.f2_duration_secs) then
               ...
               if (Configuration.exist.f2_duration_secs = 1) then
                  tmp394 := (Configuration.f2_duration_secs * 1000);
               else
                  tmp394 := ctxt.f2_duration;
               end if;
               ctxt.f2_duration := tmp394;
               ...
               --  if exist(Configuration.a1_duration_emergency_secs) then
                  ...
                  if (Configuration.exist.a1_duration_emergency_secs = 1) then
                     tmp404 := Configuration.a1_duration_emergency_secs;
                  else
                     tmp404 := ctxt.a1_duration_emergency;
                  end if;
                  ctxt.a1_duration_emergency := tmp404;
                  --  a1_duration_max :=
                  --  if exist(Configuration.a1_duration_max_secs) then
                     ...
                     if (Configuration.exist.a1_duration_max_secs = 1) then
                        tmp412 := Configuration.a1_duration_max_secs;
                     else
                        tmp412 := ctxt.a1_duration_max;
                     end if;
                     ctxt.a1_duration_max := tmp412;
                     --  a2_duration_max :=
                     --  if exist(Configuration.a2_duration_max_secs) then
                        ...
                        if (Configuration.exist.a2_duration_max_secs = 1) then
                           tmp420 := Configuration.a2_duration_max_secs;
                        else
                           tmp420 := ctxt.a2_duration_max;
                        end if;
                        ctxt.a2_duration_max := tmp420;
                  ...
                  function p_0_updateData return asn1SccT_Boolean is
                     ...
                     begin
                        ...
                        tmp433 := asn1Sccpt1000s;
                        RI_0_RetreiveSingleData(tmp433, pt1000s_data);
                        --  retreiveSingleData(ps1, press1_data) (332,17)
                        tmp437 := asn1Sccps1;
                        RI_0_RetreiveSingleData(tmp437, press1_data);
                        --  retreiveSingleData(ps2, press2_data) (334,17)
                        tmp441 := asn1Sccps2;
                        RI_0_RetreiveSingleData(tmp441, press2_data);
                        --  retreiveSingleData(heater2, power_data) (336,17)
                        tmp445 := asn1Sccheater2;
                        RI_0_RetreiveSingleData(tmp445, power_data);
                        --  temp_air := 0.0 (338,17)
                        ctxt.temp_air := 0.0;
                        ...
                        if ((((power_ok and (placa_abajo_ok or placa_arriba_ok)) and aire_infinito_ok) and (press1_ok or press2_ok))) = true then
                           ...
                           if placa_arriba_ok then
                              tmp515 := pt1000s_data.pt1000s.data.celsius.Data(6);
                           else
                              tmp515 := 0.0;
                           end if;
                           ctxt.temp_plate := tmp515;
                           ...
                           if placa_abajo_ok then
                              tmp525 := (pt1000s_data.pt1000s.data.celsius.Data(7) + ctxt.temp_plate);
                           else
                              tmp525 := ctxt.temp_plate;
                           end if;
                           ctxt.temp_plate := tmp525;
                           ...
                           if (placa_abajo_ok and placa_arriba_ok) then
                              tmp537 := (ctxt.temp_plate / 2.0);
                           else
                              tmp537 := ctxt.temp_plate;
                           end if;
                           ctxt.temp_plate := tmp537;
                           --  press := if press1_ok then float(press1_data.ps1.data.processed.pressure) /100.0 else 0.0 fi (360,0)
                           if press1_ok then
                              tmp546 := (Asn1Real(press1_data.ps1.data.processed.pressure) / 100.0);
                           else
                              tmp546 := 0.0;
                           end if;
                           ctxt.press := tmp546;
                           ...
                           if press2_ok then
                              tmp558 := ((Asn1Real(press2_data.ps2.data.processed.pressure) / 100.0) + ctxt.press);
                           else
                              tmp558 := ctxt.press;
                           end if;
                           ctxt.press := tmp558;
                           ...
                           case trId is
                              ...
                              when 3 =>
                                 ...
                                 if (((ctxt.htl_time >= ctxt.a2_duration_max) or (ctxt.press <= ctxt.press_18km))) = false then
                                    ...
                                    tmp71 := ctxt.f1_duration;
                                    SET_f1_timeout (tmp71);
                              ...
                              when 6 =>
                                 ...
                                 tmp93 := ctxt.f2_duration;
                                 SET_f2_timeout (tmp93);
                              ...
                              when 8 =>
                                 ...
                                 tmp100 := ctxt.f2_duration;
                                 SET_f2_timeout (tmp100);
                              ...
                              when 11 =>
                                 ...
                                 tmp113 := ctxt.f1_duration;
                                 SET_f1_timeout (tmp113);
