...
void heattransferlab_RI_RESET_f1_timeout(void)
{
      ...
      if (1 == innerMsc) {
         ...
         puts(""); // add newline
         // Log message to x86_partition_Timer_Manager (corresponding PI: RESET_heattransferlab_f1_timeout)
         printf ("INNER: heattransferlab,x86_partition_timer_manager,reset_f1_timeout,%lld\n", msc_time);
         fflush(stdout);
      }
   #endif
...
void heattransferlab_RI_RESET_f2_timeout(void)
{
      ...
      if (1 == innerMsc) {
         long long msc_time = getTimeInMilliseconds();
         puts(""); // add newline
         // Log message to x86_partition_Timer_Manager (corresponding PI: RESET_heattransferlab_f2_timeout)
         printf ("INNER: heattransferlab,x86_partition_timer_manager,reset_f2_timeout,%lld\n", msc_time);
         fflush(stdout);
      }
   #endif
      ...
      (const asn1SccT_UInt32 *IN_val)
{
      ...
      if (1 == innerMsc) {
         ...
         puts(""); // add newline
         // Log message to x86_partition_Timer_Manager (corresponding PI: SET_heattransferlab_f1_timeout)
         printf ("INNER: heattransferlab,x86_partition_timer_manager,set_f1_timeout,%lld\n", msc_time);
         fflush(stdout);
      }
   #endif
      ...
      (const asn1SccT_UInt32 *IN_val)
{
      ...
      if (1 == innerMsc) {
         ...
         puts(""); // add newline
         // Log message to x86_partition_Timer_Manager (corresponding PI: SET_heattransferlab_f2_timeout)
         printf ("INNER: heattransferlab,x86_partition_timer_manager,set_f2_timeout,%lld\n", msc_time);
         fflush(stdout);
      }
   #endif
   // Encode parameter Val
...
void heattransferlab_RI_setOnOffH2
      (const asn1SccHeater_On_Off *IN_cmd);
void heattransferlab_RI_setOnOffH2
      (const asn1SccHeater_On_Off *IN_cmd)
{
   #ifdef __unix__
      ...
      if (1 == innerMsc) {
         long long msc_time = getTimeInMilliseconds();
         PrintASN1Heater_On_Off ("INNERDATA: setonoffh2::Heater_On_Off::cmd", IN_cmd);
         puts(""); // add newline
         // Log message to EquipmentHandlersMock (corresponding PI: setOnOffH2)
         printf ("INNER: heattransferlab,equipmenthandlersmock,setonoffh2,%lld\n", msc_time);
         fflush(stdout);
      }
   #endif
   // Encode parameter cmd
   static asn1SccHeater_On_Off IN_buf_cmd;
   int size_IN_buf_cmd =
      Encode_NATIVE_Heater_On_Off
        ((void *)&IN_buf_cmd,
          sizeof(asn1SccHeater_On_Off),
          (asn1SccHeater_On_Off *)IN_cmd);
   if (-1 == size_IN_buf_cmd) {
      #ifdef __unix__
         puts ("[ERROR] ASN.1 Encoding failed in heattransferlab_RI_setOnOffH2, parameter cmd");
      #endif
        /* Crash the application due to message loss */
        abort();
   }
   ...
   // Call Middleware interface
   extern void vm_heattransferlab_setonoffh2
     (void *, size_t);

   vm_heattransferlab_setonoffh2
     ((void *)&IN_buf_cmd, (size_t)size_IN_buf_cmd);
      ...
      (const asn1SccT_Float *IN_power)
{
      ...
      if (1 == innerMsc) {
         ...
         puts(""); // add newline
         // Log message to EquipmentHandlersMock (corresponding PI: setPowerH2)
         printf ("INNER: heattransferlab,equipmenthandlersmock,setpowerh2,%lld\n", msc_time);
         fflush(stdout);
      }
   #endif
