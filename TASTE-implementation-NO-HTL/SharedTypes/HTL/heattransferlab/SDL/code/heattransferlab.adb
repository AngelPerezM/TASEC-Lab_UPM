...
package body Heattransferlab is
   ...
   function p_0_updateData return asn1SccT_Boolean is
      ...
      tmp252 : asn1SccT_Float;
      tmp273 : asn1SccT_Float;
      tmp261 : asn1SccT_Float;
      tmp287 : asn1SccT_Float;
      tmp230 : asn1SccT_Float;
      --  !! stack: _call_external_function line 1604
      tmp165 : asn1SccOBSW_DP_Filter;
      tmp169 : asn1SccOBSW_DP_Filter;
      tmp240 : asn1SccT_Float;
      tmp161 : asn1SccOBSW_DP_Filter;
      begin
         --  getTime(gps_time, htl_time) (62,17)
         RI_0_getTime(gps_time, ctxt.htl_time);
         --  retreiveSingleData(pt1000s,pt1000s_data) (64,17)
         tmp161 := asn1Sccpt1000s;
         RI_0_RetreiveSingleData(tmp161, pt1000s_data);
         --  retreiveSingleData(ps1, press1_data) (66,17)
         tmp165 := asn1Sccps1;
         RI_0_RetreiveSingleData(tmp165, press1_data);
         --  retreiveSingleData(ps2, press2_data) (68,17)
         tmp169 := asn1Sccps2;
         RI_0_RetreiveSingleData(tmp169, press2_data);
         --  temp_air := 0.0 (70,17)
         ctxt.temp_air := 0.0;
         --  temp_plate := 0.0 (71,0)
         ctxt.temp_plate := 0.0;
         ...
         if ((((placa_abajo_ok or placa_arriba_ok) and aire_infinito_ok) and (press1_ok or press2_ok))) = true then
            ...
            if placa_arriba_ok then
               tmp230 := pt1000s_data.pt1000s.data.celsius.Data(6);
            else
               tmp230 := 0.0;
            end if;
            ctxt.temp_plate := tmp230;
            ...
            if placa_abajo_ok then
               tmp240 := (pt1000s_data.pt1000s.data.celsius.Data(7) + ctxt.temp_plate);
            else
               tmp240 := ctxt.temp_plate;
            end if;
            ctxt.temp_plate := tmp240;
            ...
            if (placa_abajo_ok and placa_arriba_ok) then
               tmp252 := (ctxt.temp_plate / 2.0);
            else
               tmp252 := ctxt.temp_plate;
            end if;
            ctxt.temp_plate := tmp252;
            --  press := if press1_ok then float(press1_data.ps1.data.processed.pressure) /100.0 else 0.0 fi (90,0)
            if press1_ok then
               tmp261 := (Asn1Real(press1_data.ps1.data.processed.pressure) / 100.0);
            else
               tmp261 := 0.0;
            end if;
            ctxt.press := tmp261;
            ...
            if press2_ok then
               tmp273 := ((Asn1Real(press2_data.ps2.data.processed.pressure) / 100.0) + ctxt.press);
            else
               tmp273 := ctxt.press;
            end if;
            ctxt.press := tmp273;
   ...
   procedure configureParameters(Configuration: in out asn1SccHTL_Config) is
      tmp534 : asn1SccT_Float;
      tmp550 : asn1SccT_Float;
      tmp586 : asn1SccT_Double;
      tmp542 : asn1SccT_Float;
      tmp578 : asn1SccT_Double;
      tmp558 : asn1SccT_UInt32;
      tmp568 : asn1SccT_UInt32;
      tmp594 : asn1SccT_Double;
      begin
         ...
         if (Configuration.exist.press_5km = 1) then
            tmp534 := Configuration.press_5km;
         else
            tmp534 := ctxt.press_5km;
         end if;
         ctxt.press_5km := tmp534;
         ...
         if (Configuration.exist.press_10km = 1) then
            tmp542 := Configuration.press_10km;
         else
            tmp542 := ctxt.press_10km;
         end if;
         ctxt.press_10km := tmp542;
         ...
         if (Configuration.exist.press_18km = 1) then
            tmp550 := Configuration.press_18km;
         else
            tmp550 := ctxt.press_18km;
         end if;
         ctxt.press_18km := tmp550;
         --  f1_duration :=
         --   if exist(Configuration.f1_duration_secs) then
            ...
            if (Configuration.exist.f1_duration_secs = 1) then
               tmp558 := (Configuration.f1_duration_secs * 1000);
            else
               tmp558 := ctxt.f1_duration;
            end if;
            ctxt.f1_duration := tmp558;
            --  f2_duration :=
            --  if exist(Configuration.f2_duration_secs) then
               ...
               if (Configuration.exist.f2_duration_secs = 1) then
                  tmp568 := (Configuration.f2_duration_secs * 1000);
               else
                  tmp568 := ctxt.f2_duration;
               end if;
               ctxt.f2_duration := tmp568;
               ...
               --  if exist(Configuration.a1_duration_emergency_secs) then
                  ...
                  if (Configuration.exist.a1_duration_emergency_secs = 1) then
                     tmp578 := Configuration.a1_duration_emergency_secs;
                  else
                     tmp578 := ctxt.a1_duration_emergency;
                  end if;
                  ctxt.a1_duration_emergency := tmp578;
                  --  a1_duration_max :=
                  --  if exist(Configuration.a1_duration_max_secs) then
                     ...
                     if (Configuration.exist.a1_duration_max_secs = 1) then
                        tmp586 := Configuration.a1_duration_max_secs;
                     else
                        tmp586 := ctxt.a1_duration_max;
                     end if;
                     ctxt.a1_duration_max := tmp586;
                     --  a2_duration_max :=
                     --  if exist(Configuration.a2_duration_max_secs) then
                        ...
                        if (Configuration.exist.a2_duration_max_secs = 1) then
                           tmp594 := Configuration.a2_duration_max_secs;
                        else
                           tmp594 := ctxt.a2_duration_max;
                        end if;
                        ctxt.a2_duration_max := tmp594;
                  ...
                  procedure Execute_Transition (Id : Integer) is
                     ...
                     tmp55 : asn1SccT_UInt32;
                     tmp153 : asn1SccT_UInt32;
                     tmp42 : asn1SccT_UInt32;
                     tmp117 : asn1SccT_UInt32;
                     begin
                        while (trId /= -1) loop
                           case trId is
                              when 0 =>
                              ...
                              when 3 =>
                                 ...
                                 tmp42 := ctxt.f2_duration;
                                 SET_f2_timeout (tmp42);
                              ...
                              when 6 =>
                                 ...
                                 tmp55 := ctxt.f1_duration;
                                 SET_f1_timeout (tmp55);
                              ...
                              when 16 =>
                                 ...
                                 tmp117 := ctxt.f2_duration;
                                 SET_f2_timeout (tmp117);
                              ...
                              when 18 =>
                                 ...
                                 if (((ctxt.htl_time >= ctxt.a2_duration_max) or (ctxt.press <= ctxt.press_18km))) = false then
                                    ...
                                    tmp153 := ctxt.f1_duration;
                                    SET_f1_timeout (tmp153);
