...
def setMsgQ():
    global tmId
    tmId = PythonController.i_Environmental_Data


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_Environmental_Data
    tmId = i_gui_PI_Environmental_Data


def Environmental_Data(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.ENV_GUI_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.ENV_GUI_REQUIRED_BYTES_FOR_ENCODING)
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ Environmental-Data ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received Environmental-Data with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected Environmental-Data (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for Environmental-Data') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type ENV_GUI
    # Environmental_Data = ASN1.ENV_GUI()
    return None
