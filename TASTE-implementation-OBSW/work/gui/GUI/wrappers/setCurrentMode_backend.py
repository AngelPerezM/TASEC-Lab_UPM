...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_setCurrentMode
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gui_RI_setCurrentMode
    tcId = i_gui_RI_setCurrentMode
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global setCurrentMode_via_shared_lib
    shared_lib = True
    setCurrentMode_via_shared_lib = dll.gui_PI_setCurrentMode
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.HTL_State_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.HTL_State_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_setCurrentMode(tc)
            except IOError:
                raise
    elif udp:
