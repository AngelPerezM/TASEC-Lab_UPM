...
def setMsgQ():
    global tmId
    tmId = PythonController.i_Attitude_Data


def setUDP():
    global tmId
    from _InterfaceEnum import i_gui_PI_Attitude_Data
    tmId = i_gui_PI_Attitude_Data


def Attitude_Data(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.ATT_GUI_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.ATT_GUI_REQUIRED_BYTES_FOR_ENCODING)
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ Attitude-Data ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received Attitude-Data with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected Attitude-Data (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for Attitude-Data') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type ATT_GUI
    # Attitude_Data = ASN1.ATT_GUI()
    return None
